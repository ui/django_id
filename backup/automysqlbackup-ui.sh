...
# Email Address to send mail to? (teddy@ui.co.id)
MAILADDR="teddy@ui.co.id"
...
echo ======================================================================
	...
	if [ ! -e "$BACKUPDIR/mysql/$DB" ]		# Check Daily DB Directory exists.
		then
		mkdir -p "$BACKUPDIR/mysql/$DB"
	fi
	...
	# Daily Backup
		...
		eval rm -fv "$BACKUPDIR/mysql/$DB/*.$DOW.sql.*" 
		echo
			dbdump "$DB" "$BACKUPDIR/mysql/$DB/${DB}.sql"
			compression "$BACKUPDIR/mysql/$DB/${DB}.sql"
			BACKUPFILES="$BACKUPFILES $BACKUPDIR/mysql/$DB/${DB}.sql$SUFFIX"
		echo ----------------------------------------------------------------------
	fi
	done
...
echo ======================================================================
	...
	# Daily Backup
		...
		echo
		echo Rotating last weeks Backup...
		eval rm -fv "$BACKUPDIR/mysql/*.$DOW.sql.*" 
		echo
			dbdump "$DBNAMES" "$BACKUPDIR/mysql/$DATE.$DOW.sql"
			compression "$BACKUPDIR/mysql/$DATE.$DOW.sql"
			BACKUPFILES="$BACKUPFILES $BACKUPDIR/mysql/$DATE.$DOW.sql$SUFFIX"
		echo ----------------------------------------------------------------------
	fi
